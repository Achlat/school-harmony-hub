  ...
  return (
    <div className="space-y-6">
              ...
              <tbody>
                {HOURS.map(hour => (
                  <tr key={hour} className="border-b border-border/50 last:border-0">
                    <td className="px-3 py-2 text-xs font-medium text-muted-foreground">{hour}</td>
                    {DAYS.map((_, dayIdx) => {
                      ...
                      return (
                        <td key={dayIdx} className="px-1 py-1">
                          {slotCourses.map(c => (
                            <div
                              key={c.id}
                              className="mb-1 cursor-pointer rounded-lg px-2 py-1.5 text-xs transition-opacity hover:opacity-80"
                              style={{
                                backgroundColor: slotBg(c.color),
                                color: c.color ?? COLORS[0],
                                borderLeft: `3px solid ${c.color ?? COLORS[0]}`,
                              }}
                              onClick={() => openEdit(c)}
                            >
      ...
      <ConfirmDialog
        ...
        description={`Voulez-vous vraiment supprimer ${deleteTarget?.subject} — ${deleteTarget?.className} ?`}
      />
    </div>
  );
}
